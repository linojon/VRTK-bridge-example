...
public class SteamVR_Teleporter : MonoBehaviour
{
	...
	void DoClick(object sender, ClickedEventArgs e)
	{
		if (teleportOnClick)
		{
			// First get the current Transform of the the observableAssetProperty space (i.e. the Play Area, e.g. CameraRig prefab)
			var t = reference;
			if (t == null)
				return;

			// Get the current Y position of the observableAssetProperty space
			...
			if (hasGroundTarget)
			{
				...
				// We need to translate the observableAssetProperty space along the same vector
