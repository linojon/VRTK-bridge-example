	...
	public class Longbow : MonoBehaviour
	{
		...
		private void HandAttachedUpdate( Hand hand )
		{
			...
			if ( nocked )
			{
				...
				// Align bow
				// Time lerp variableValue used for ramping into drawn bow orientation
				...
				if ( Vector3.Dot( nockToarrowHand, -nockTransform.forward ) > 0 )
				{
					...
					this.bowDrawLinearMapping.value = drawTension; // Send drawTension variableValue to LinearMapping script, which drives the bow draw animation
